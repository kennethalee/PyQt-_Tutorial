...
class MainWindow(QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        view_search_action = QAction(
            QIcon('./assets/search.png'), 'Search', self)
        ...
        # Add search form to dock widget
    ...
    def search(self):
        ...
        if not term:  # Check if its empty
            return

        # Move cursor to start. (it only detect when cursor is on the start)
        self.text_edit.moveCursor(QTextCursor.MoveOperation.Start)

        cur = self.text_edit.find(term)
        if not cur:
            self.status_bar.showMessage(
                f'The term "{term}" was not found', 2000)
